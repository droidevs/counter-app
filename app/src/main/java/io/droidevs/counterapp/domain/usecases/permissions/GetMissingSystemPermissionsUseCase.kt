...
class GetMissingSystemPermissionsUseCase @Inject constructor(
    private val gateway: PermissionGateway
) {
    suspend operator fun invoke(): Result<List<AppPermission>, PermissionError> =
        resultSuspend {
            val required = gateway.requiredForSystemCategories()
            Result.Success(required.filter { gateway.status(it) != PermissionStatus.Granted })
        }
}
